...
class GenAttack2(object):
    def mutation_op(self,  cur_pop, idx, step_noise=0.01, p=0.005):
        perturb_noise = tf.random.uniform(cur_pop.get_shape(),
                                          minval=-step_noise, maxval=step_noise, dtype=tf.float32)
        mutated_pop = perturb_noise * \
            tf.cast(tf.random.uniform(cur_pop.get_shape())
    ...
    def attack_step(self, idx, success, orig_copies, cur_noise, prev_elite, margin_log, best_win_margin, cur_plateau_count, num_plateaus):
        ...
        if self.resize_dim:
            noise_resized = tf.image.resize(
                cur_noise, (self.image_dim, self.image_dim), method=tf.image.ResizeMethod.BILINEAR)
        else:
        ...
        all_preds = tf.argmax(input=pop_preds, axis=1)

        success_pop = tf.cast(tf.equal(all_preds, self.target), tf.int32)
        success = tf.reduce_max(input_tensor=success_pop, axis=0)

        target_scores = tf.reduce_sum(input_tensor=self.tlab * pop_preds, axis=1)
        sum_others = tf.reduce_sum(input_tensor=(1-self.tlab) * pop_preds, axis=1)
        max_others = tf.reduce_max(input_tensor=(1-self.tlab) * pop_preds, axis=1)

        # the goal is to maximize this loss
        loss = -(tf.math.log(sum_others+1e-30) - tf.math.log(target_scores+1e-30))

        win_margin = tf.reduce_max(
            input_tensor=pop_preds[:, self.target] - tf.reduce_max(input_tensor=pop_preds, axis=1))

        new_best_win_margin, new_cur_plateau_count = tf.cond(
            pred=tf.greater(win_margin, best_win_margin),
            false_fn=lambda: (best_win_margin, cur_plateau_count+1),
            true_fn=lambda: (win_margin, 0)
        )
        plateau_threshold = tf.cond(pred=tf.greater(win_margin, -0.40),
                                    true_fn=lambda: 100,
                                    false_fn=lambda: 300)
        new_num_plateaus, new_cur_plateau_count = tf.cond(
            pred=tf.greater(new_cur_plateau_count, plateau_threshold),
        ...
        if self.adaptive:
            ...
            step_p = tf.cond(pred=tf.less(idx, 10),
                             true_fn=lambda: 1.0,
        ...
        elite_idx = tf.cond(
            pred=tf.equal(success, 1),
            true_fn=lambda: tf.expand_dims(
                tf.cast(tf.argmax(input=success_pop), tf.int32), axis=0),
            false_fn=lambda: tf.expand_dims(tf.cast(tf.argmax(input=loss, axis=0), tf.int32), axis=0))

        elite = tf.gather(cur_noise, elite_idx)
        select_probs = tf.nn.softmax(tf.squeeze(loss) / step_temp)
        parents = tfp.distributions.Categorical(
        ...
        xover_prop = tf.cast(tf.random.uniform(
            shape=parent1.get_shape()) > pp2, tf.float32)
        childs = parent1 * xover_prop + parent2 * (1-xover_prop)
        idx = tf.compat.v1.Print(idx+1, [idx, tf.reduce_min(input_tensor=loss),
    ...
    def __init__(self, model, pop_size=6, mutation_rate=0.001,
        ...
        self.tlab = tf.one_hot(
            [self.target], depth=self.num_labels)
        ...
        # __import__('ipdb').set_trace()
        self.attack_main = tf.while_loop(cond=cond, body=attack_body, loop_vars=[self.i, self.init_success,  self.pop_orig,
                                                             self.pop_noise,
                                                             self.pop_noise[0],
                                                             self.margin_log,
                                                             ...
                                                             self.num_plateaus],
                                                             shape_invariants=[self.margin_log.get_shape(), tf.TensorShape([None, 1])])
    ...
    def initialize(self, sess, img, target):
        ...
        sess.run([x.initializer for x in
                  [self.i,
                   ...
                   self.num_plateaus,
                   self.init_success]])
        sess.run(tf.compat.v1.assign(self.input_img, np.expand_dims(img, axis=0)))
        sess.run(tf.compat.v1.assign(self.target, target))
        orig_copies = tf.tile(self.input_img, [self.pop_size, 1, 1, 1])
        sess.run(tf.compat.v1.assign(self.pop_orig, orig_copies))
        init_noise = self.mutation_op(
            self.pop_noise, idx=self.i, p=self.mutation_rate, step_noise=self.eps)
        sess.run(tf.compat.v1.assign(self.margin_log, np.zeros((1, 1), dtype=np.float32)))
        sess.run(tf.compat.v1.assign(self.pop_noise, init_noise))
        sess.run(tf.compat.v1.assign(self.best_win_margin,
                           np.array(-1.0, dtype=np.float32)))
        sess.run(tf.compat.v1.assign(self.cur_plateau_count, np.array(0, dtype=np.int32)))
        sess.run(tf.compat.v1.assign(self.num_plateaus, 0))
    ...
    def attack(self, sess, input_img, target_label):
        ...
        if success:
            if self.resize_dim:
                adv_img = sess.run(
                    tf.clip_by_value(
                        np.expand_dims(input_img, axis=0)+tf.image.resize(
                            np.expand_dims(adv_noise, axis=0), (self.image_dim, self.image_dim), method=tf.image.ResizeMethod.BILINEAR),
